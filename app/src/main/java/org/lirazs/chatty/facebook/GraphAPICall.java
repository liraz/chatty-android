...
public class GraphAPICall {
    ...
    private void setUp (GraphAPICallback callback) {
        ...
        if (!isAccessTokenValid()) {
            Log.e(GraphAPICall.TAG, "Cannot call Graph API without a valid AccessToken");
        }
    }
    ...
    public static GraphAPICall callMeFriends(String fields, GraphAPICallback callback) {
        if (isPermissionGranted(FacebookLoginPermission.USER_FRIENDS)) {
            GraphAPICall call = new GraphAPICall(callback);
            call.createMyFriendsRequest();
            call.addParam(PARAM_FIELDS, fields);
            return call;
        } else {
            Log.w(TAG, "Cannot call me/friends without user_friends permission");
            return null;
        }
    }
    ...
    public static GraphAPICall callAppScores(String appId, GraphAPICallback callback) {
        if (isPermissionGranted(FacebookLoginPermission.USER_FRIENDS)) {
            ...
            Log.w(TAG, "Cannot call app-id/scores without user_friends permisison");
            return null;
        }
    }
    ...
    public static GraphAPICall publishScore (int score, GraphAPICallback callback) {
        if (isPermissionGranted(FacebookLoginPermission.PUBLISH_ACTIONS)) {
            GraphAPICall call = new GraphAPICall(callback);
            call.createPublishScoreRequest(score);
            return call;
        } else {
            Log.w(TAG, "Cannot publish scores without publish_actions permission");
            return null;
        }
    }
